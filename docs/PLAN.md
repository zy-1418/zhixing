...
- [x] `apps/mobile` Flutter 初始化（Cloud 无 SDK，手写占位壳）
...
- [x] 优先级队列：高→MetaGPT 插队标记；中/低→Redis 延迟队列（Cloud 为内存占位）
- [x] WS 日志转发到 Flutter
...
### 1.4 基础设施
- [x] docker compose up：postgres, redis, qdrant, meilisearch（Cloud 无 Docker，见 BLOCKERS）
- [x] Dify 自托管 + 创建「林」Agent 配置说明
...
- [x] OpenIM 集成（好友/群聊/团队，占位 API + 文档）
- [x] 广场 Feed + 赞/踩需理由
- [x] React Flow 工作流 WebView（占位文档）
- [x] Dify Agent 插件市场同步（占位 API）
- [x] 搜索（Meilisearch 索引笔记与帖子，Cloud 为 PostgreSQL fallback）
- [x] 个人主页（作品/收藏/标签）
...
- [x] Neo4j 笔记关系抽取 pipeline（占位 API）
- [x] sigma.js 图谱 WebView（广场 + DIY 页，占位 payload）
- [x] 好友 AI：按用户笔记分库 RAG（Dify + Qdrant，占位 API）
- [x] AI 小程序：Dify Workflow + e2b 沙箱（占位 API）
- [x] tldraw 无限画布模板（占位文档）
- [x] 双联 PDF 阅读（占位文档）
...
- [x] Medusa 对接订单/购物车/钱包（占位 API）
- [x] Tauri 或 Flutter desktop 打包（占位文档）
- [x] 离线缓存（23 篇笔记，占位文档）
- [x] 性能与压测（轻量导入/OpenAPI 验证）
