...
- 公开任务状态与重试接口：`GET /api/v1/tasks/{identifier}`、`POST /api/v1/tasks/{identifier}/retry`，支持知行任务 UUID 或 MetaGPT job_id。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，任务状态/重试接口在本地数据库或 MetaGPT 不可用时返回 `blocked` 响应，外部服务实际联调需在本机执行。
