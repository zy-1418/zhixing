...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应；任务状态查询、QA 重试、日志转发等 MetaGPT 相关端点在 Cloud 中会返回 `blocked` 结构化说明，外部服务实际联调需在本机执行。
