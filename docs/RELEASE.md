...
- MetaGPT SOP 提交、任务状态查询、QA optimize 重试、任务日历、优先级队列占位与 WS 日志代理。
...
with TestClient(app) as client:
    assert client.get("/api/v1/tasks/metagpt-unavailable-test").json()["blocked"] is True
    assert client.post("/api/v1/tasks/metagpt-unavailable-test/retry").json()["blocked"] is True
print("openapi ok")
PY
```
