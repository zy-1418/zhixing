...
公开任务合约已覆盖 `GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry`，`identifier` 可为知行任务 UUID 或 MetaGPT job_id。Cloud 无法连接 PostgreSQL/MetaGPT-X 时接口返回 `blocked` 占位载荷而不是中断验收。
