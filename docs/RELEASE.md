...
- 公开任务状态/重试契约：`GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry`，支持知行任务 UUID 或 MetaGPT job_id。
...
任务状态与重试端点在 PostgreSQL 或 MetaGPT-X 不可用时返回 `blocked`/`blocked_reason`，便于前端展示阻塞而不中断工作流。
