...
- MetaGPT SOP 提交、公开任务状态查询、QA retry/optimize、任务日历、优先级队列占位、WS 日志代理。
...
- `GET /api/v1/tasks/{identifier}` 支持按知行任务 UUID 或 MetaGPT job id 查询任务状态。
- `POST /api/v1/tasks/{identifier}/retry` 支持按同一 identifier 触发 MetaGPT optimize 重试。
- 当本地 PostgreSQL 或 MetaGPT-X 在 Cloud 不可用时，上述端点返回 `blocked`/占位信息，便于前端保持同一合约。
