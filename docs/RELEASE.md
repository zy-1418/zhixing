...
- MetaGPT SOP 提交、公共任务状态查询、QA 重试代理、任务日历、优先级队列占位、WS 日志代理。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，外部服务实际联调需在本机执行。任务状态与重试接口会先尽力查询本地 PostgreSQL，再代理 MetaGPT-X；当任一依赖不可达时返回 `blocked` 说明而不中断验收。
