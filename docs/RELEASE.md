...
- 公共任务契约：`GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry` 支持知行任务 UUID 或 MetaGPT job id。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应；任务提交、状态查询与 retry 在 MetaGPT 不可达时返回 `blocked` 信息，外部服务实际联调需在本机执行。
