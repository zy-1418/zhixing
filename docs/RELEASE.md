...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应；任务状态/重试端点在 MetaGPT-X 或 PostgreSQL 不可用时返回 `blocked` 详情，外部服务实际联调需在本机执行。
