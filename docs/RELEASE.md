...
公共任务契约已补齐：`GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry` 均支持知行任务 UUID 或 MetaGPT job_id；Cloud 中 PostgreSQL/MetaGPT-X 不可达时返回 `blocked` 占位结果。
