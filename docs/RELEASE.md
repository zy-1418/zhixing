...
- 公共任务查询/重试契约：`GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry` 支持知行任务 UUID 或 MetaGPT job_id。
...
任务状态与重试接口在 PostgreSQL 或 MetaGPT-X 不可用时返回 `blocked=true` 与具体 `blocked_reason`，便于 Flutter 继续展示占位状态。
