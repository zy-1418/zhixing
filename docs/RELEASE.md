...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，`/api/v1/tasks/sop`、`/api/v1/tasks/{identifier}`、`/api/v1/tasks/{identifier}/retry` 会在外部依赖不可达时返回 blocked 占位信息，外部服务实际联调需在本机执行。
