...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应；`/api/v1/tasks/{identifier}` 与 `/api/v1/tasks/{identifier}/retry` 会在远端不可达时返回 `blocked`，外部服务实际联调需在本机执行。
