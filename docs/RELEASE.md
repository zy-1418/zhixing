...
- MetaGPT SOP 提交、公开状态查询/QA 重试、任务日历、优先级队列占位、WS 日志代理。
...
任务公开契约已覆盖 `GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry`。`identifier` 可为知行任务 UUID 或 MetaGPT job_id；Cloud 下 PostgreSQL / MetaGPT-X 不可达时返回 `blocked` 与原因，便于前端继续展示任务状态。
