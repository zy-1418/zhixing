...
- MetaGPT SOP 提交、公开任务状态查询、QA 重试封装、任务日历、优先级队列占位、WS 日志代理。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，外部服务实际联调需在本机执行。`GET /api/v1/tasks/{identifier}` 与 `POST /api/v1/tasks/{identifier}/retry` 在 PostgreSQL 或 MetaGPT-X 不可用时会返回 `blocked` 占位状态，便于前端展示阻塞原因。
