...
- MetaGPT SOP 提交、任务状态查询、公共 retry/QA optimize、任务日历、优先级队列占位、WS 日志代理。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，`/api/v1/tasks/{identifier}` 与 `/api/v1/tasks/{identifier}/retry` 在外部服务不可用时返回 blocked 详情，实际联调需在本机执行。
