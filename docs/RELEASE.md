...
- 公开任务状态与重试契约：`GET /api/v1/tasks/{identifier}`、`POST /api/v1/tasks/{identifier}/retry`，支持知行任务 UUID 或 MetaGPT job id。
...
Cursor Cloud 缺少 Docker、Flutter SDK，且无法访问开发者本机 `127.0.0.1:8000` MetaGPT-X。因此本次实现保留 API 契约与占位响应，任务状态查询和重试在 Cloud 中会返回 `blocked` 原因，外部服务实际联调需在本机执行。
...
for path in ["/api/v1/tasks/{identifier}", "/api/v1/tasks/{identifier}/retry"]:
    assert path in paths, path
print("openapi ok")
PY
```
