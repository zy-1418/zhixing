...
- `GET /api/v1/tasks/{identifier}` 支持用知行任务 UUID 或 MetaGPT job id 查询状态。
- `POST /api/v1/tasks/{identifier}/retry` 支持触发 MetaGPT optimize 重试。
- PostgreSQL 或 MetaGPT-X 在 Cloud 不可用时，上述端点返回 `blocked=true` 与原因说明，保持前端可展示状态。
