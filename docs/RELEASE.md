...
- MetaGPT SOP 提交、公开任务状态查询、QA retry 封装、任务日历、优先级队列占位、WS 日志代理。
...
任务状态与重试端点支持使用知行任务 UUID 或 MetaGPT job id：

- `GET /api/v1/tasks/{identifier}`
- `POST /api/v1/tasks/{identifier}/retry`

当 PostgreSQL 或 MetaGPT-X 在 Cloud 中不可用时，上述端点返回 `blocked=true`、`blocked_reason` / `db_error`，用于保留客户端契约并继续工作流。
