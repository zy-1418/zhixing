...
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；任务状态与 retry 端点返回 `blocked=true` 与失败原因 |
...
| PostgreSQL | Cloud 未启动本地数据库 | 任务状态与 retry 端点将跳过本地任务绑定，仍按传入 identifier 尝试代理 MetaGPT-X 并返回 blocked 占位 |
