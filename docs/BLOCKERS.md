...
任务公共状态与重试接口（`GET /api/v1/tasks/{identifier}`、`POST /api/v1/tasks/{identifier}/retry`）会先尝试查询本地 `tasks` 表，再代理 MetaGPT-X。Cloud 中 PostgreSQL 或 MetaGPT-X 不可用时，接口返回 `blocked=true` 和 `blocked_reason`，保留前端与自动化验收契约。
