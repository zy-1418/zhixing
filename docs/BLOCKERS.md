...
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；`GET /api/v1/tasks/{task_id}` 与 `POST /api/v1/tasks/{task_id}/retry` 返回 `blocked=true` 与 `blockers[]` |
...
5. 运行后端导入、OpenAPI、任务状态/重试、Flutter analyze 与端到端任务提交检查。
