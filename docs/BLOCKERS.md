...
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；`/api/v1/tasks/{identifier}` 与 `/api/v1/tasks/{identifier}/retry` 返回 `blocked` 状态与原因 |
| PostgreSQL | 未启动本地数据库服务 | 任务状态/重试端点以 best-effort 查询本地任务，数据库不可用时仍按 MetaGPT job id 返回占位响应 |
