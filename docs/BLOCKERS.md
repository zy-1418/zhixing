...
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；`/api/v1/tasks/{identifier}`、`/api/v1/tasks/{identifier}/retry` 与日志 WS 返回 `blocked` 结构化说明 |
