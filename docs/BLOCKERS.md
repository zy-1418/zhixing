...
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；`/api/v1/tasks/{identifier}` 与 `/api/v1/tasks/{identifier}/retry` 返回 blocked 占位状态 |
| Dify | 未配置 `DIFY_API_KEY`，且未启动自托管服务 | `/api/v1/dify/*` 返回 blocked/echo 占位响应 |
| PostgreSQL / Redis / Meilisearch / Qdrant / Neo4j / Medusa | 中间件未启动 | 暴露 API 契约与配置项，任务查询采用 best-effort 本地查找，服务不可用时返回 placeholder/blocked 状态 |
