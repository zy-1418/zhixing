...
| PostgreSQL | Cloud 未启动本地数据库 | 任务状态/重试接口捕获查询失败并返回 `blocked` 占位负载 |
| MetaGPT-X `127.0.0.1:8000` | Cloud 无法访问本机服务 | `/api/v1/tasks/sop` 在连接失败时返回 `metagpt-unavailable-*` 占位 job_id；状态查询与重试接口返回 `blocked` |
