...
def main() -> None:
    ...
    missing = sorted(EXPECTED_HTTP_PATHS - paths.keys())
    if missing:
        raise SystemExit(f"missing OpenAPI paths: {missing}")

    websocket_paths = {getattr(route, "path", "") for route in tasks_router.routes}
    if "/tasks/{job_id}/logs" not in websocket_paths:
        raise SystemExit("missing task log websocket: /tasks/{job_id}/logs")
