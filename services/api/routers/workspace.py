...
async def _folder_tree_response(user_id: uuid.UUID, db: AsyncSession) -> list[dict[str, Any]]:
...
async def _conversation_export_response(
...
async def folders_tree_alias(
    user_id: uuid.UUID = Query(...),
    db: AsyncSession = Depends(get_db),
):
    return await _folder_tree_response(user_id, db)
...
async def export_conversation(
    ...
    return await _conversation_export_response(conversation_id, format, db)
...
async def export_conversation_json(
    ...
    return await _conversation_export_response(conversation_id, "json", db)


@router.get("/conversations/export.md")
async def export_conversation_markdown(
    conversation_id: uuid.UUID = Query(...),
    db: AsyncSession = Depends(get_db),
):
    return await _conversation_export_response(conversation_id, "markdown", db)
