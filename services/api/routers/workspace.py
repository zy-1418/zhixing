...
async def _folder_tree_payload(user_id: uuid.UUID, db: AsyncSession) -> list[dict[str, Any]]:
...
async def folder_tree_by_resource(
    user_id: uuid.UUID = Query(...), db: AsyncSession = Depends(get_db)
):
    return await _folder_tree_payload(user_id, db)
