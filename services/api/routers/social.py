...
contract_router = APIRouter(tags=["social-contract"])
...
@contract_router.post("/debates", status_code=201)
async def create_debate_contract(body: DebateCreate):
    return await create_debate(body)


@contract_router.get("/debates/{debate_id}")
async def get_debate_contract(debate_id: str):
    return await get_debate(debate_id)


@contract_router.post("/debates/{debate_id}/comments", status_code=201)
async def add_debate_comment_contract(debate_id: str, body: DebateCommentCreate):
