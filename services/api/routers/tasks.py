...
async def get_task_status(identifier: str, db: AsyncSession = Depends(get_db)):
    task, lookup_error = await _lookup_task(identifier, db)
    ...
    blocked_reason = _join_reasons(lookup_error, remote_error)
...
async def retry_task(
    ...
    if task is not None and not task.metagpt_job_id:
        reason = _join_reasons(lookup_error, "Task has no MetaGPT job id")
    ...
    try:
        ...
        return PublicTaskRetryResponse(
            ...
            blocked_reason=_join_reasons(
                lookup_error, f"MetaGPT-X optimize unavailable: {e}"
            ),
            qa_fix_rounds=qa_fix_rounds,
        )

    status = str(optimize.get("status") or "running")
    if task is not None:
        task.status = status
        try:
            await db.commit()
