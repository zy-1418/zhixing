...
async def get_task_status(identifier: str, db: AsyncSession = Depends(get_db)):
    task, db_error = await _find_task_by_identifier(identifier, db)
    ...
    if task is not None:
        ...
        return PublicTaskStatusResponse(
            ...
            local=local,
            remote=remote,
            blocked=remote_error is not None,
            blocked_reason=remote_error,
        )

    if remote is not None:
        status = (
            str(remote.get("status", "unknown"))
    ...
    return PublicTaskStatusResponse(
        ...
        status="blocked",
        blocked=True,
        blocked_reason=_join_blockers(db_error, remote_error, "Task not found locally"),
    )
...
async def retry_task(
    identifier: str,
    qa_fix_rounds: int = Query(3, ge=1, le=20),
    db: AsyncSession = Depends(get_db),
):
    task, db_error = await _find_task_by_identifier(identifier, db)
    job_id = task.metagpt_job_id if task and task.metagpt_job_id else identifier
    ...
    if task is not None and not task.metagpt_job_id:
        return TaskRetryResponse(
            ...
            status="blocked",
            blocked=True,
            blocked_reason="Task has no MetaGPT job id yet",
        )
    ...
    try:
        ...
        return TaskRetryResponse(
            ...
            status="blocked",
            blocked=True,
            blocked_reason=_join_blockers(
                db_error,
                f"MetaGPT-X optimize unavailable: {e}",
                None if task else "Task not found locally",
            ),
        )

    status = (
        str(optimize.get("status"))
        if isinstance(optimize, dict) and optimize.get("status")
        else "queued"
    )
    if task is not None:
        task.status = status
        ...
        try:
            ...
            return TaskRetryResponse(
                ...
                optimize=optimize,
                blocked=True,
                blocked_reason=f"PostgreSQL retry metadata update failed: {e}",
            )
