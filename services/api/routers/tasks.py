...
async def _resolve_task_identifier(
    ...
    Database access is best-effort because Cursor Cloud often runs without the
    local PostgreSQL stack. In that case callers can still proxy the identifier
    directly to MetaGPT-X and return a documented blocked placeholder.
    """
    task: Task | None = None
    try:
        try:
        ...
        if task is None:
            stmt = select(Task).where(Task.metagpt_job_id == identifier)
        ...
        try:
            ...
            pass
        return None, identifier, f"PostgreSQL task lookup unavailable: {e}"

    if task is not None:
        return task, task.metagpt_job_id or identifier, None
...
async def get_task_status(identifier: str, db: AsyncSession = Depends(get_db)):
    task, job_id, db_blocked_reason = await _resolve_task_identifier(identifier, db)
    client = MetaGPTClient(base_url=settings.metagpt_x_api)
    metagpt: dict[str, Any] | None = None
    metagpt_blocked_reason: str | None = None

    if job_id:
        try:
            metagpt = await client.get_project(job_id)
        except Exception as e:
            metagpt_blocked_reason = f"MetaGPT-X status unavailable: {e}"

    reasons = [r for r in (db_blocked_reason, metagpt_blocked_reason) if r]
    status = (
        str(metagpt.get("status"))
        if metagpt and metagpt.get("status") is not None
        else task.status
        if task is not None
        else "blocked"
        if reasons
...
async def retry_task(
    identifier: str, qa_fix_rounds: int = 3, db: AsyncSession = Depends(get_db)
):
    task, job_id, db_blocked_reason = await _resolve_task_identifier(identifier, db)
    client = MetaGPTClient(base_url=settings.metagpt_x_api)
    optimize: dict[str, Any] | None = None
    ...
    if job_id:
        try:
            ...
            metagpt_blocked_reason = f"MetaGPT-X optimize unavailable: {e}"
    else:
        metagpt_blocked_reason = "Task has no MetaGPT job id"

    if task is not None and optimize is not None:
        task.status = str(optimize.get("status") or "queued")
        task.metadata_ = {
            **(task.metadata_ or {}),
    ...
    reasons = [r for r in (db_blocked_reason, metagpt_blocked_reason) if r]
    status = (
        str(optimize.get("status"))
        if optimize and optimize.get("status") is not None
        else task.status
        if task is not None
        else "blocked"
    )
