...
def _task_to_local_payload(task: Task) -> dict[str, Any]:
    return {
        "id": str(task.id),
...
async def get_task_status(identifier: str, db: AsyncSession = Depends(get_db)):
    ...
    local_payload = _task_to_local_payload(task) if task is not None else None
    ...
    blocked_reason = remote_error or db_error
    ...
    return TaskStatusResponse(
        ...
        status=remote_status or local_status or ("blocked" if blocked_reason else "unknown"),
        local=local_payload,
        remote=remote,
