...
async def retry_task(
    ...
    if not task.metagpt_job_id:
        return {
            "blocked": True,
            "task_id": str(task_id),
            "reason": "Task is not bound to a MetaGPT-X job yet.",
        }

    result = await optimize_metagpt_job(task.metagpt_job_id, qa_fix_rounds)
    task.metadata_ = {
        **(task.metadata_ or {}),
        "last_retry": {
    ...
    if not result.get("blocked"):
        task.status = "queued"
    await db.commit()
    return {"task_id": str(task_id), "metagpt_job_id": task.metagpt_job_id, "result": result}
